(() => {
    ...
    function initSliders() {
        if (document.querySelector(".proffer__slider")) {
            const screenWidth = window.innerWidth;
            const sliderElement = document.querySelector(".proffer__slider");
            const wrapperElement = document.querySelector(".proffer__wrapper");
            if (screenWidth <= 767.98) if (!sliderElement.classList.contains("swiper-initialized")) {
                wrapperElement.classList.add("swiper-wrapper");
                sliderElement.classList.add("swiper");
                new swiper_core_Swiper(".proffer__slider", {
                    modules: [ Pagination ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: "auto",
    ...
    function windowLoaded() {
        let documentActions = e => {
            ...
            if (targetElement.closest(".item-dashboard__more")) {
                const parentItem = targetElement.closest(".dashboard__item");
                if (activeItem && activeItem !== parentItem) activeItem.classList.remove("--active");
                parentItem.classList.toggle("--active");
                if (parentItem.classList.contains("--active")) activeItem = parentItem; else activeItem = null;
            }
        };
        ...
        document.querySelectorAll(".item-dashboard__more");
        if (dashboardItems.length > 0) {
            activeItem = dashboardItems[4];
        ...
        if (!hoursEl || !minutesEl || !secondsEl) {
            console.warn("⏳ Таймер не знайдено на сторінці!");
            return;
        }
        let time = 10 * 60;
        function updateCountdown() {
            ...
            if (time === 0) time = 10 * 60; else time--;
        }
        updateCountdown();
        setInterval(updateCountdown, 1e3);
        const profferSlides = document.querySelectorAll(".proffer__slide");
        profferSlides.forEach((slide => {
            const width = slide.offsetWidth;
            console.log(width);
            if (width > 270) {
                slide.classList.add("wide-slide");
                console.log("Цей слайд ширший за 270px:", slide);
            }
        }));
